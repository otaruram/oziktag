...
export function ApiDocsModal({ onClose }: { onClose: () => void }) {
  ...
  "product_id": "uuid-string"
            ...
            {/* Tracking Lite Endpoint */}
            <div>
              <div className="flex items-center gap-3 mb-4">
                <span className="bg-primary/20 text-primary px-2 py-1 rounded text-xs font-bold font-mono">POST</span>
                <code className="text-sm font-mono font-medium">/api/v1/tracking</code>
              </div>
              <p className="text-muted-foreground mb-4">Endpoint ini digunakan untuk men-generate resi digital Tracking Lite.</p>
  ...
  "checklist": ["String", "String"] (Array of Strings, Required),
  "catatan_penjual": "String (Optional)",
  "image_url": "String (HTTP URL, Optional)"
}`}</pre>
              </div>
              ...
              <div className="bg-muted p-3 rounded-md overflow-x-auto text-xs font-mono border border-border/50 mb-4">
                <pre>{`{
  "tracking_url": "https://www.oziktag.my.id/tracking/...",
  "buyer_pin": "A1B2C3D4",
  "product_id": "uuid-string"
}`}</pre>
              </div>
            ...
            <div>
              <h4 className="font-semibold mb-2">Error Responses (Common)</h4>
